 ...
 *
 * `transaction_manager.ts` owns the stack, the savepoints and the callback
 * lists. `after_commit.ts` owns the callbacks themselves. This is the smaller
 ...
 *
 * Named for the outcome rather than `isRestorable`, which `transaction_manager.ts`
 ...
 *
 * Validated through `transaction_manager.ts`'s check rather than a second
