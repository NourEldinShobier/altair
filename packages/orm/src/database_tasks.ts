 ...
 *
 * `schema_tasks.ts` has the pieces — which tables are dumpable, whether the
 * schema is up to date, whether this environment may be destroyed.
 * `database_configurations.ts` has the configurations. This is the layer that
...
import { type DatabaseConfiguration, databaseTasks } from "./database_configurations.js";
import { checkProtectedEnvironment } from "./schema_tasks.js";
 ...
 * `MigrationContext`, which `migration_context.ts` already uses for the object
 * that runs them.
 */
