...
| -------------------- | ----- | -------------------------------------------------------------------------- |
| `@altair/support`    | 1326  | Inflector, callbacks, cache, i18n, logging, durations, time zones          |
| `@altair/orm`        | 1768  | Connection, migrations, models, relations, associations, ActiveModel       |
| `@altair/controller` | 701   | Filters, strong params, rendering, dispatch, cookies, sessions, CSRF, i18n |
| `@altair/cli`        | 191   | Generators, db tasks, file loading, encrypted credentials                  |
| `@altair/router`     | 113   | Resourceful routing, typed path helpers                                    |
| `@altair/cable`      | 138   | Action Cable, protocol-compatible with Rails' client                       |
| `@altair/storage`    | 230   | Disk and S3 services, blobs, attachments, variants, direct uploads         |
| `@altair/view`       | 466   | TSX rendering, layouts, Inertia protocol, form builders                    |
| `@altair/jobs`       | 149   | Jobs, queues, retries, worker                                              |
| `@altair/testing`    | 219   | Transactional tests, fixtures, factories, test databases                   |
| `@altair/core`       | 170   | Config, boot lifecycle, request handler, credentials, logging              |
| `@altair/mailer`     | 195   | Messages, TSX bodies, delivery methods                                     |
...
What remains everywhere is depth: 21.2% of Rails' test count, not 21.2% of Rails.
