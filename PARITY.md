...
| -------------------- | ----- | -------------------------------------------------------------------------- |
| `@altair/support`    | 1547  | Inflector, callbacks, cache, i18n, logging, durations, time zones          |
| `@altair/orm`        | 2104  | Connection, migrations, models, relations, associations, ActiveModel       |
| `@altair/controller` | 862   | Filters, strong params, rendering, dispatch, cookies, sessions, CSRF, i18n |
...
| `@altair/cable`      | 208   | Action Cable, protocol-compatible with Rails' client                       |
| `@altair/storage`    | 320   | Disk and S3 services, blobs, attachments, variants, direct uploads         |
| `@altair/view`       | 555   | TSX rendering, layouts, Inertia protocol, form builders                    |
| `@altair/jobs`       | 219   | Jobs, queues, retries, worker                                              |
...
| `@altair/mailer`     | 280   | Messages, TSX bodies, delivery methods                                     |
...
What remains everywhere is depth: 25.4% of Rails' test count, not 25.4% of Rails.
